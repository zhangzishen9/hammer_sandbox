...
# 确保基础工具存在
apt update -y && apt install -y wget curl jq openssl python3 python3-pip python3-cryptography bc build-essential
...
scripts=("menu.sh" "core.sh" "install_sb.sh" "config_gen.sh" "warp_pool.sh" "warp_rotate.sh" "re-assemble.sh" "protocol_manager.sh" "hammer_bench.sh" "subscription_server.sh" "subscription_manager.py" "hammer_stats.go")
